    ...
    <div className="flex flex-col items-center">
      ...
      >
        ...
        <p className="text-xl md:text-3xl font-bold uppercase tracking-widest text-white/80 max-w-2xl mx-auto border-y-2 border-accent py-2 bg-black/40 backdrop-blur-sm">
          Ultimate PDF Manipulation <span className="text-accent italic">Extreme Edition</span>
        </p>
      </motion.div>
          ...
          >
            <Link href={`/tools/${tool.id}`}>
              <div className="manga-card group p-8 flex flex-col items-center text-center h-full cursor-pointer overflow-hidden">
                ...
                <div className={`p-4 mb-6 rounded-full border-4 border-black ${tool.color} text-white group-hover:scale-110 transition-transform`}>
                  {tool.icon}
                </div>
          ...
          <div className="max-w-xl">
            ...
            <ul className="space-y-4 font-bold text-lg">
              <li className="flex gap-4 items-start">
                <span className="bg-accent text-white p-1 mt-1 text-xs">01</span>
                <span>Lightning fast processing with local execution. No server wait.</span>
              </li>
              <li className="flex gap-4 items-start">
                <span className="bg-accent text-white p-1 mt-1 text-xs">02</span>
                <span>Your files never leave your browser. Total privacy.</span>
              </li>
              <li className="flex gap-4 items-start">
                <span className="bg-accent text-white p-1 mt-1 text-xs">03</span>
                <span>The only PDF editor that gives you main character energy.</span>
              </li>
            </ul>
          </div>
          <div className="flex flex-col gap-4">
            <button className="manga-button text-2xl">
              Get Started Now!
            </button>
            <p className="text-center text-xs uppercase tracking-widest text-white/40">No registration required</p>
          </div>
        </div>
      </motion.div>
    </div>
